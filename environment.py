# MarketEnvironment 类以及与市场环境相关的逻辑
            ...
            if r.is_active:
                if r.cumulative_utility < CONFIG['RESTAURANT_EXIT_THRESHOLD']:
                    ...
                    self.platform.commission_rate = max(CONFIG['MIN_COMMISSION'],
                                                        self.platform.commission_rate * (1 - CONFIG['EMERGENCY_SUBSIDY_RATE']))
    ...
    def simulate_period(self):
        ...
        for consumer in self.consumers:
            ...
            if demand > 0:
                ...
                cons_utility = consumer.calculate_utility(consumer.value * demand, order_value + self.platform.delivery_fee,
                ...
                consumer_satisfaction.append(np.mean(consumer.satisfaction_history) if consumer.satisfaction_history else 0.5)
